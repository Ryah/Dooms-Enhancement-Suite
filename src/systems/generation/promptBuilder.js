...
export function generateTrackerExample() {
    ...
    if (extensionSettings.showQuests && committedTrackerData.quests) {
        try {
            JSON.parse(committedTrackerData.quests);
            const lockedData = applyLocks(committedTrackerData.quests, 'quests');
            parts.push(`  "quests": ${lockedData}`);
        } catch {
...
// Helper to extract value from potentially locked fields and common object formats
        ...
        if (trackerType === 'quests') {
            // Quests (independent top-level tracker)
            // Main quest - handle string, array, or object with {title}
            if (data.main) {
                if (typeof data.main === 'string') {
                    const mainQuest = getValue(data.main);
                    if (mainQuest && mainQuest !== 'None') formatted += `Main Quest: ${mainQuest}\n`;
                } else if (Array.isArray(data.main) && data.main.length > 0) {
                    const questsList = data.main.map(q => getValue(q)).filter(q => q);
            ...
            // Optional quests
            if (data.optional && Array.isArray(data.optional) && data.optional.length > 0) {
                const questsList = data.optional.map(q => getValue(q)).filter(q => q);
        ...
        console.warn('[Dooms Tracker] Failed to format tracker data for context:', e);
        console.warn('[Dooms Tracker] Error details:', e.stack);
        return ''; // Return empty string on error to avoid breaking context
    }
}
/**
 * Formats historical tracker data from a message's dooms_tracker_swipes data.
 ...
 *
 * @param {Object} trackerData - The tracker data from message.extra.dooms_tracker_swipes[swipeId]
    ...
    try {
        ...
        if (trackerData.quests) {
            ...
            if (shouldIncludeQuests) {
                const questsData = typeof trackerData.quests === 'string'
                    ? JSON.parse(trackerData.quests)
                    : trackerData.quests;
                let questsFormatted = '';
                if (questsData.main) {
                    const mainQuest = getValue(questsData.main);
                    if (mainQuest && mainQuest !== 'None') questsFormatted += `Quest: ${mainQuest}, `;
                }
                if (questsData.optional && Array.isArray(questsData.optional) && questsData.optional.length > 0) {
                    const questsList = questsData.optional.map(q => getValue(q)).filter(q => q);
                    if (questsList.length > 0) questsFormatted += `Side Quests: ${questsList.join(', ')}, `;
                }
                if (questsFormatted) {
                    formatted += `Quests: ${questsFormatted.slice(0, -2)}\n`;
                }
            }
        }
        // Backward compatibility: Process quests nested inside userStats from legacy data
        if (!trackerData.quests && trackerData.userStats) {
            const userStatsData = typeof trackerData.userStats === 'string'
                ? JSON.parse(trackerData.userStats)
                : trackerData.userStats;
            if (userStatsData.quests) {
                const questsConfig = trackerConfig.quests;
                const shouldIncludeQuests = useAllEnabled || questsConfig?.persistInHistory;
                if (shouldIncludeQuests) {
        ...
        console.warn('[Dooms Tracker] Failed to format historical tracker data:', e);
        return '';
    }
}
/**
...
export function generateContextualSummary() {
    ...
    const userName = getContext().name1;
    let summary = '';
    // Add Quests tracker data if enabled
    if (extensionSettings.showQuests && committedTrackerData.quests) {
        try {
            const formatted = formatTrackerDataForContext(committedTrackerData.quests, 'quests', userName);
            if (formatted) {
                summary += formatted + '\n';
            }
        } catch (e) {
            console.warn('[Dooms Tracker] Failed to format quests for context:', e);
        }
    }
    ...
    if (extensionSettings.showInfoBox && committedTrackerData.infoBox) {
        try {
            ...
            console.warn('[Dooms Tracker] Failed to format infoBox for context:', e);
        }
    }
    ...
    if (hasAnyPreviousData) {
        ...
        if (extensionSettings.showQuests && committedTrackerData.quests) {
            try {
                ...
                unifiedPrevious.quests = parsed;
            } catch {
                promptText += `${committedTrackerData.quests}\n\n`;
            }
        }
        if (extensionSettings.showInfoBox && committedTrackerData.infoBox) {
            try {
